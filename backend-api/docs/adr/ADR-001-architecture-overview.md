...
- **Worker**: Responsible for asynchronous job orchestration, background processing, and coordination between system components.
...
- Incremental evolution toward a microservices-oriented architecture based on real domain boundaries and system requirements.

External infrastructure components, such as the Message Broker and object storage providers, must be accessed through abstractions to allow future replacement with minimal impact on business logic.
...
Future ADRs will define additional architectural decisions, including:
